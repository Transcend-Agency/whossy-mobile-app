...
//
Future<T?> showSnapModelBottomSheet<T>({
...
class _CupertinoModalTransition extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (Theme.of(context).platform == TargetPlatform.iOS && paddingTop > 20) {
      ...
      //https://kylebashour.com/snapps/finding-the-real-iphone-x-corner-radius
    }
...
class CupertinoScaffold extends StatefulWidget {
  static _CupertinoScaffold? _of(BuildContext context) =>
  ...
  static Future<T?> showCupertinoModalBottomSheet<T>({
    ...
    final topRadius = CupertinoScaffold._of(context)!.topRadius;
    final result =
        await Navigator.of(context, rootNavigator: useRootNavigator).push(
      CupertinoModalBottomSheetRoute<T>(
        closeProgressThreshold: closeProgressThreshold,
        builder: builder,
        secondAnimationController: CupertinoScaffold._of(context)!.animation,
        containerBuilder: (context, _, child) => _CupertinoBottomSheetContainer(
          backgroundColor: backgroundColor,
